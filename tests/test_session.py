...
def test_header_round_trips_workspace_model_title_and_window(tmp_path):
    store = SessionStore.create(tmp_path, workspace=str(tmp_path), model="fake-model", context_window=128_000, title="Demo page")
...
def test_projection_does_not_duplicate_audit_tool_call(tmp_path):
    ...
    store.append_new("tool_result", {"result": ToolResult(tool_call_id="c1", tool_name="read_file", ok=True, content="x")}, turn_id="t1")
    assert [item.message.role for item in store.project_messages()] == ["assistant", "tool"]
